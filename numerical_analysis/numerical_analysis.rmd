...
outer product between two vectors \(\mathbf{a}, \mathbf{b}\) to be the matrix 
\(\mathbf{a}\mathbf{b}^T = A \in M_n(\mathbb{R})\) where \(A_{ij} = a_ib_j\). 
We also note that the dot product on the reals form a bilinear form, and so 
all associated properties apply.
...
we shall, in fact, look at two methods of achieving the QR-decomposition, hence 
the name of this section. The QR-decomposition decomposes a matrix into the product 
of two matrices \(Q\) and \(R\) where \(Q\) is orthogonal and \(R\) is upper 
triangular. 
