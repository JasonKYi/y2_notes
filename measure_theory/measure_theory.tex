...
\hypertarget{results-in-measure-theory}{%
\section{Results in Measure Theory}\label{results-in-measure-theory}}
...
\(d_i < \text{diam } B_0 \le d_{i - 1}\). By maximality of
\(\mathcal{G}_i\), there exists
\(B \in \mathcal{G}_1 \cup \cdots \cup  \mathcal{G}_i\) with
\(B \cap B_0 \neq \varnothing\). In particular, \(\text{diam} B > d_i\)
and since \(\text{diam } B_0 \le d_{i - 1} = 2d_i\), it follows
...
\hypertarget{decomposition-of-measures}{%
\subsection{Decomposition of Measures}\label{decomposition-of-measures}}

Let \((X, \mathcal{A}, \mu)\) be a \(\sigma\)-finite measure space,
\(f : X \to [0, \infty]\) measurable. As we have seen \(\nu := f\mu\),
i.e.~\(\nu(A) = \int_A f \dd \mu\), defines a measure on
\((X, \mathcal{A})\). As a consequence, \(\nu \ll \mu\), that is,
\(\nu\) is absolutely continuous with respect to \(\mu\). In this
section we investigate to which extent every absolutely continuous
measure \(\nu \ll \mu\) is in the form \(\nu = f \mu\).
...
By noting that we have excluded \(-\infty\) from our codomain, we are
allowed to continue with our arithmetic on the extended reals as normal.

If we were to have \(\mu_1, \mu_2\) measures on \((X, \mathcal{A})\)
with \(\mu_2(X) < \infty\). Then, we may define
\[\mu(A) := \mu_1(A) - \mu_2(A),\] for all \(A \in \mathcal{A}\). It is
not difficult to see that \(\mu\) is a signed measure, and as we shall
see, it turns out every signed measure can be written in this form.
...
Let \(B \subseteq A_1 \cup A_2\). Then
\(B = (B \cap A_1) \sqcup (B \cap A_1^c)\), and so
\(\alpha(B) = \alpha(B \cap A_1) + \alpha(B \cap A_1^c)\). Clearly,
\(B \cap A_1 \subseteq A_1\) and
\(B \cap A_1^c = B \setminus A_1 \subseteq A_2\). Hence, we have
\[\alpha(B) = \alpha(B \cap A_1) + \alpha(B \cap A_1^c) \ge 0 + 0 = 0.\]
Similar proof for negative sets. \qed

It is not difficult to generalise this proposition to countable unions
of positive sets which proof is left as an exercise.
...
\((A_1, B_1)\) and \((A_2, B_2)\) are two Hahn decompositions for the
signed measure \(\alpha\), then the symmetric difference between \(A_1\)
and \(A_2\) is a null-set. Indeed, we have
\[A_1 \Delta A_2 = (A_1 \cap A_2^c) \cup (A_2 \cap A_1^c) = 
  (B_1^c \cap B_2) \cup (B_2^c \cap B_1) = B_1 \Delta B_2.\] Now, since
\(A_1 \Delta A_2 \subseteq A_1 \cup A_2\) which is positive, we have
\(\alpha(A_1 \Delta A_2) \ge 0\). On the other hand,
\(A_1 \Delta A_2 = B_1 \Delta B_2 \subseteq B_1 \cup B_2\) which is
negative, we have \(\alpha(A_1 \Delta A_2) \le 0\). Hence
\(\alpha(A_1 \Delta A_2) = 0\).
...
Let
\(\epsilon_1 := \sup \{\alpha(A) \mid A \subseteq B, A \in \mathcal{A}\}\).
By properties of the supremum, there exists some \(A_1\) such that
\(\alpha(A_1) \ge \epsilon_1 / 2 \wedge 1\). Proceeding inductively,
assuming \(\epsilon_1, A_1, \cdots, \epsilon_{n - 1}, A_{n - 1}\), then
we define
\[\epsilon_n := \sup \left\{\alpha(A) \mid A \subseteq B \setminus \bigcup_{i = 1}^{n - 1} A_i, 
    A \in \mathcal{A} \right\},\] and choose
\(A_n \subseteq B \setminus \bigcup_{i = 1}^{n - 1} A_i\) so
\(\alpha(A_n) \ge \epsilon_n / 2 \wedge 1\). Clearly, by construction,
\((A_n)\) is a disjoint sequence and
\(\alpha(A_n) \ge \epsilon_n / 2 \wedge 1\). Now, define
\[B' := B \setminus \bigcup_{i = 1}^\infty A_n,\] and so, we have
\[0 > \alpha(B) = \alpha(B') + \alpha\left(\bigcup A_n\right) 
    = \alpha(B') + \sum \alpha(A_n) \ge \alpha(B').\] Moreover, since
\(\alpha(B') > -\infty\), we have \(\sum \alpha(A_n) < \infty\), and so
\(\epsilon_n \le 2\alpha(A_n) \to 0\) as \(n \to \infty\). Now, let
\(A \subseteq B'\), for all \(n \in \mathbb{N}\), we have
\(A \subseteq B \setminus \bigcup_{k = 1}^{n - 1} A_k\) and so,
\(\alpha(A) \le \epsilon_n\). Thus taking \(n \to \infty\) we have
\(\alpha(A) \le 0\) and hence, \(B'\) is negative. \qed

Finally, we may move to prove the Hahn decomposition theorem.

\proof (Hahn decomposition). Let
\(b := \inf\{ \alpha(B) \mid B \in \mathcal{A}, B \text{ negative.}\}\).
By definition, there exists some \(B_n\) negative sets, \(n \ge 1\) such
that \(\alpha(B_n) \downarrow b\). Then, taking the union of these sets,
we found a negative set \(B := \bigcup B_n\), and in particular
\(b \le \alpha(B)\). Moreover, for every \(n\),
\[\alpha(B) = \alpha(B_n) + \alpha(B \setminus B_n) \le \alpha(B_n).\]
Thus, by letting \(n \to \infty\), we have \(\alpha(B) \le b\) and so,
we have \(b = \alpha(B)\) and in particular, \(b > -\infty\).

Now, let \(A := B^c\) and we will show \(A\) is positive. If
\(A' \subseteq A\), \(A \in \mathcal{A}\) with \(\alpha(A') < 0\), by
the above lemma, there exists a subset \(A'' \subseteq A'\) that is
negative. But then \(B \sqcup A''\) is negative and so,
\[\alpha(B \sqcup A'') = \alpha(B) + \alpha(A'') < \alpha(B) = b,\]
which contradicts the minimality of \(b\). Thus, such an \(A'\) does not
exist and \(A\) is positive. \qed
...
\(\alpha_+(E) := \alpha(E \cap A)\) and
\(\alpha_-(E) := -\alpha(E \cap B)\) for all \(E \in \mathcal{A}\).
Since \(A \cup B = X\) and \(A, B\) are disjoint, it follows that
\(\alpha_+ - \alpha_- = \alpha\). Moreover, since \(A, B\) are positive,
negative respectively, \(\alpha_{\pm}\) are measures, and
\(\alpha_+(A^c) = 0\) whereas
\(\alpha_-(A) = - \alpha(B \cap B^c) = 0\). Thus,
\(\alpha_+ \bot \alpha_-\).

It remains to show uniqueness. Suppose \(\alpha = \beta_+ - \beta_-\)
with \(\beta_{\pm}\) are mutually singular. Let \(D\) be the set such
that \(\beta_+(D) = 0 = \beta_-(D^c)\). So, for all subsets of \(D\),
\(\tilde D\), we have
\(\alpha(\tilde D) = \beta_+(\tilde D) - \beta_-(\tilde D) = - \beta_-(\tilde D) \le 0\),
...
\[\alpha_+(E) = \alpha(E \cap A) = \alpha(E \cap (A \cup D^c)) 
    = \alpha(E \cap D^c) = \beta_+(E),\] and hence,
\(\alpha_+ = \beta_+\). Similarly, we have \(\alpha_- = \beta_-\). \qed

\hypertarget{radon-nikodym-theorem}{%
\subsection{Radon-Nikodym Theorem}\label{radon-nikodym-theorem}}
...
\(\nu_{ac}\) is the absolutely continuous part. Indeed, we see that
\(\nu_{ac} \ll \mu\).
...
For \(n \in \mathbb{N}\), let \((A_n, B_n)\) be the Hahn-decomposition
for \(\mu - \frac{1}{n}\nu\). Set \(A = \bigcup A_n\) and
\(B = \bigcap B_n\), and then, since \(B \subseteq B_n\), \(B\) is a
negative set for \(\mu - \frac{1}{n}\nu\), i.e.~
\[\mu(B) \le \frac{1}{n}\nu(B), \text{for all } n \in \mathbb{N}.\]
Since \(\nu\) is finite, taking \(n \to \infty\), we have
\(\mu(B) = 0\). If \(\mu\) is not mutually singular with \(nu\), then
\(\nu(B^c) = \nu(A) > 0\), whence \(\nu(A_{n_0}) > 0\) for some \(n_0\)
and so, the result follows by choosing \(\epsilon = \frac{1}{n_0}\) and
\(E = A_{n_0}\). \qed

\proof (Lebesgue Decomposition). We first show existence. We may assume
\(\mu\) and \(\nu\) are finite. Indeed, in the general case, writing
\(X = \bigcup S_i\) for disjoint sets \(S_i \in \mathcal{A}\), with
\(\mu(S_i), \nu(S_i) < \infty\), we can define
\(\mu_i(\cdot) = \mu(\cdot \cap S_i)\) and
\(\nu_i(\cdot) = \nu(\cdot \cap S_i)\), and decomposing
\(\nu_i = \nu_{i, s} + \nu_i{i, ac}\), one obtain that
\(\nu_s := \sum \nu_{i, s}\) and \(\nu_{ac} := \sum \nu_{i, ac}\) forms
...
Clearly \(\mathcal{G} \neq \varnothing\) since \(0 in \mathcal{G}\).
Furthermore, it is closed under maximum. Indeed, if
\(g, h \in \mathcal{G}\), then, for all \(A \in \mathcal{G}\),
\[\int_A(g \vee h) \dd \mu = \int_{A \cap \{g > h\}} g \dd \mu 
    + \int_{A \cap \{g \le h\}} h \dd \mu \le \nu(A \cap \{g > h\}) + 
    \nu(A \cap \{g \le h\}) = \nu(A),\] and, hence,
\(g \vee h \in \mathcal{G}\). Furthermore, we define
\[M := \sup \left\{\int g \dd \mu \mid g \in \mathcal{G}\right\} \in [0, \infty).\]
By the definition of \(M\), there exists some
\((g_n) \subseteq \mathcal{G}\) such that,
\[\lim_{n \to \infty} \int g_n \dd \mu = M.\] So, setting
\(f_n := \bigvee_{i = 1}^n g_i\) for all \(n \in \mathbb{N}\), we have a
monotonically increasing sequence of \(\mathcal{G}\), \((f_n)\), and
\(f := \lim_{n \to \infty} f_n\) exists. By monotone convergence, we
have
\[\int_A f \dd \mu = \lim_{n \to \infty} \int_A f_n \dd \mu \le \nu(A),\]
for all \(A \in \mathcal{A}\) and hence, \(f \in \mathcal{G}\). Thus, we
...
\(\nu_s \bot \mu\). By the above lemma, if the two measures are not
mutually singular, then there exists some \(\epsilon > 0\),
\(E \in \mathcal{A}\), such that \(\nu_s(E) > 0\) and \(E\) is positive
for \(\nu - f \dd \mu - \epsilon \mathbf{1}_E \dd \mu\). Hence,
\[\int_{E \cap A} (f + \epsilon) \dd \mu \le \nu(A \cap E).\] But this
implies that for all \(A \in \mathcal{A}\),
\[\int_A (f + \epsilon \mathbf{1}_E) \dd \mu = \int_{A \cap E} (f + \epsilon) \dd \mu + 
    \int_{A \cap E^c} f \dd \mu \le \nu(A),\] and so,
\(f + \epsilon \mathbf{1}_E \in \mathcal{G}\), contradicting the
maximality of \(\int f \dd \mu = M\). Hence, \(\nu_s \bot \mu\).

Lastly, it remains to show uniqueness. Assume
\(\nu = \nu_s' + \nu_{ac}'\) be a Lebesgue decomposition, then
\(\nu_s - \nu_s' = \nu_{ac}' - \nu_{ac}\). Since, \(\nu_s \bot \mu\) and
\(\nu_s' \bot \mu\), there exists some \(B \in \mathcal{A}\) such that
\(\nu_s(B^c) = \nu_s'(B^c) = 0\) and \(\mu(B) = 0\). Thus,
\[(\nu_s - \nu_s')(A) = (\nu_s - \nu_s')(A \cap B) = (\nu_{ac}' - \nu_{ac})(A \cap B) = 0,\]
where the last equality follows since \(\mu(A \cap B) = 0\) and so,
\(\nu_{ac}(A \cap B) = \nu_{ac}'(A \cap B) = 0\) as both are absoultely
continuous with resepct to \(\mu\). \qed
...
We have already shown the reverse implication so let us consider the
forward implication.

Suppose \(\nu \ll \mu\) and write \(\nu = \nu_s + \nu_{ac}\) for the
Lebesgue decomposition of \(\nu\). Since \(\nu_s \bot \mu\), there
exists some \(E \in \mathcal{A}\) such that \(\nu_s(E) = 0\) and
\(\mu(E^c) = 0\). However, since \(\nu \ll \mu\), this implies that
\(\nu(E^c) = 0\), and so,
\[\nu_s(E^c) = \nu(E^c) - \int_E f \dd \mu \le 0.\] As, \(\nu_s\) is a
measure, this means that \(\nu_s(E^c) = 0\) and hence,
\(\nu_s(A) = \nu_s(A \cap E) + \nu_s(A \cap E^c) = 0\) for al
\(A \in \mathcal{A}\). Thus,
\(\nu = \nu_s + \nu_{ac} = \nu_{ac} = f \mu\). \qed
