...
We recall from year 1 computing the definitions of big-\(O\) and big-\(\Omega\) 
notations.
...
The clustering coefficient of a random Gilbert graph is given by \(p\). This is 
...
Consider a random Gilbert graph \(G(N, p)\). Suppose, naively, we write \(k_i\) 
...
\proof
  ...
  \((\implies)\) We will instead prove a stronger statement with the next theorem.
\qed
