...
As \(z_0 \in \partial \gamma\) where \(\partial \gamma\) is open, there
exists some \(r' > 0\) such that
\(B_{r'}(z_0) \subseteq \partial \gamma\). Furthermroe, as \(f\) is
holomorphic as \(z_0\), we have \(f\) is continuous as \(z_0\). So by
fixing \(\epsilon > 0\), there exists some \(\delta > 0\), such that the
continuity criterion applies. So, by letting
\(r :< \min\{r', \delta\}\), \(C := C_r(z_0)\) is a closed curve fully
