...
\(A = [T]_B\). Now, by permuting the standard basis by \(pi\), resulting
in the basis \(B'\), we have \(A' = [T]_{B'}\), and so, by letting \(P\)
be the change of basis matrix from \(B \to B'\), we have shown
\(A \sim A'\). \qed
...
applying induction (which is a nice exercise). \qed
