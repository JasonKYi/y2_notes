...
\(V = \bigoplus V_i\). Now, by letting \(T : V \to V : v \mapsto A v\),
we see that for all \(i\), \(V_i\) is \(T\)-invariant and
\(T_{V_i} = v \mapsto A_i v\), so, by taking the basis \(B\) to be the
standard basis, we have \(A = [T]_B\). Now, by permuting the standard
basis by \(pi\), resulting in the basis \(B'\), we have
\(A' = [T]_{B'}\), and so, by letting \(P\) be the change of basis
matrix from \(B \to B'\), we have shown \(A \sim A'\). \qed
...
linear map \(T : V \to V\) and similarly for matrices. Then, the
Cayley-Hamilton theorem states the famous result that, given a linear
map \(T : V \to V\), if \(\chi_T\) is the characteristic polynomial of
\(T\), then \(\chi_T(T) = 0\). We will prove this theorem within this
chapter.
...
\proof (Cayley-Hamilton Theorem) Let \(T : V \to T\) be a linear map and
\(\chi_T\) be the characteristic polynomial of \(T\). We will induct on
the dimension of \(T\), \(n\).
